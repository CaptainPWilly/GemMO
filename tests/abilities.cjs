...
assert(html.includes('@media (min-width:700px)'),'tablet breakpoint must exist');
assert(html.includes('orientation:landscape'),'landscape tablet layout must exist');
assert(html.includes('grid-template-areas:'),'landscape combat must use a two-pane grid');
assert(html.includes("localStorage.getItem('gemmo.session')"),'login token must persist across browser restarts');
assert(html.includes("localStorage.setItem('gemmo.session'"),'successful login must remember the session');
assert(html.includes("localStorage.removeItem('gemmo.session'"),'logout must clear the remembered session');
assert(html.includes("localDevHost?'http://127.0.0.1:8787':'https://gemmo.onrender.com'"),'localhost must default to the local API');
assert(html.includes("won:false,gold:0,xp:0"),'defeats must settle their match ticket with zero rewards');
assert(html.includes('activeRewardBudget=data.match?.rewardBudget||null'),'client must accept the server-issued reward budget');
assert(html.includes("activeRewardBudget?'/'+activeRewardBudget.gold:''"),'combat HUD must show the server Gold budget');
assert(html.includes("activeAuthority?.mode==='replay-v1'"),'Rat combat must recognize the server replay authority mode');
assert(html.includes("resultBody.transcript=combatTranscript"),'verified victories must submit the combat transcript');
assert(html.includes("makeCombatRng(activeAuthority.seed)"),'authoritative combat must use the server-issued deterministic seed');
assert(html.includes("recordCombatAction({t:'swap'"),'player swaps must enter the combat proof transcript');
assert(html.includes("recordCombatAction({t:'ability'"),'gem activations must enter the combat proof transcript');
let src=html.match(/<script>([\s\S]*?)<\/script>/)[1];src=src.replace("showScreen('splash');",`globalThis.api={setHintDelay:v=>hintDelay=v,ITEMS,GEAR,EQUIPMENT_SLOTS,WORLD_NODES,worldCanTravel,worldPath,worldCleared,sackIsValid,gearStats,playerMaxHP,reservoirCap,canEquipGear,equipGear,unequipGear,getEquipment:()=>({...equipment}),resetEquipment:()=>{equipment={...DEFAULT_EQUIPMENT}},setInventory:v=>inventory=v.slice(),setTestAccount:v=>account=v,setWorldClears:v=>worldState.clearedEncounters=v.slice(),applyTarget,afterAction,fallColumns,reshuffleBoard,touchActivity,showHint,damagePlayer,findMatches,legalMoves,reservoirCap,enemyUseActive,setEnemyReady:color=>ec[color]=ENEMY[color].cap,startFight,applyColor,activate,trySwap,tapCell,get:()=>({charges,sack,pHP,eHP,pGuard,eGuard,freeSwap,overdrive,playerTurn,board,buffs,pinColumn,pinTurns,guardTurns,evadeTurns,actionNumber,targetMode}),setHP:v=>pHP=v,setGuard:v=>eGuard=v,setBoard:v=>{board=v;render()},setTurn:v=>playerTurn=v,setSack:v=>sack=v,setReady:i=>{charges[itemById(sack[i]).color]=itemById(sack[i]).cap;playerTurn=true;pHP=10},finish:async()=>{await Promise.all(damageAnimations.splice(0))}};showScreen('splash');`);
